...
static void _enable_address_change(ADG726_t *dev) {
    dev->set_gpio_pin(dev->cs_pin, ADG726_LOW);
    dev->set_gpio_pin(dev->wr_pin, ADG726_LOW);
}

static void _set_address_pins(ADG726_t *dev, uint8_t input_pin) {
    // INPUT PIN RANGE: 0 to 15 should be checked before calling this function
    ...
    for (uint8_t addr_pin = 0; addr_pin < NB_ADDRESS_PINS; ++addr_pin) {
        ...
        dev->set_gpio_pin(dev->address_pins[addr_pin], state);
    }
}

static void _enable_input(ADG726_t *dev) {
    dev->set_gpio_pin(dev->wr_pin, ADG726_HIGH);
    dev->set_gpio_pin(dev->cs_pin, ADG726_HIGH);
}
